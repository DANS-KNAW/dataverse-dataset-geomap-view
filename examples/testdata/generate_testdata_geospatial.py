...
def generate_WGS84_coordinates_data(n, d):
    ...
    if d == 4:
        ...
        # get coordinates_data points where this is not the case and swap
...
if __name__ == '__main__':
    ...
    with open(path + '/geospatial_dataset_json.j2', 'r') as f:
         template = jinja2.Template(f.read())

    for i in range(n):
        # one box only for the datasets now, 
        # but it can have multiple!
        nr_of_boxes = np.random.randint(1, 3) # at least one box
        content = template.render(title=f'Test geospatial dataset {str(i)}  with {nr_of_boxes} box{"" if nr_of_boxes == 1 else "es"} location {id}',
                                  keyword=f'maptest {timestamp}',
                                  boxes=generate_WGS84_coordinates_data(nr_of_boxes, 4))  # generate box with 2 dimensions
